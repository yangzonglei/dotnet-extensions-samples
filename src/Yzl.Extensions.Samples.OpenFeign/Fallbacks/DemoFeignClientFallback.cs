...
namespace Yzl.Extensions.Samples.OpenFeign.Fallbacks;
...
public sealed class DemoFeignClientFallback : IDemoFeignClient
{
    ...
    public Task<UserDto> Create(CreateUserRequest request) => Task.FromResult(new UserDto(0, request.Name, request.Age, request.City));

    public Task<UserDto> Update(long id, UpdateUserRequest request) => Task.FromResult(new UserDto(id, request.Name, request.Age, request.City));
